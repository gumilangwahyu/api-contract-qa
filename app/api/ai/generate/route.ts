...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `Generate realistic mock JSON data that strictly matches this JSON Schema:\n${JSON.stringify(schema, null, 2)}\n\nContext:\n- HTTP Method: ${method || 'GET'}\n- API Path: ${path || '/'}\n- Description: ${description || ''}\n\nStrict Requirements for Mock Data Quality:\n1. All text fields, descriptions, titles, categories, addresses, messages, and names MUST be generated in Indonesian (Bahasa Indonesia):\n   - "message" / "msg": Generate natural Indonesian response/status messages (e.g., "Berhasil mengambil data", "Data ditemukan", "Sukses").\n   - "event": Generate realistic Indonesian event names (e.g., "Konser Musik Indie", "Pameran Buku Bandung", "Festival Kuliner Nusantara").\n   - "aktivitas_event": Generate detailed activity descriptions for the event (e.g., "Sesi tanya jawab dengan pembicara", "Registrasi peserta dan pembagian merchandise").\n   - "genre": Generate realistic Indonesian genre/category names (e.g., "Musik", "Edukasi", "Komedi", "Teknologi").\n   - "gambar" / image/photo fields: Generate real, valid placeholder or stock image URLs (e.g., "https://picsum.photos/600/400", "https://images.unsplash.com/photo-1501854140801-50d01698950b", "https://placehold.co/600x400"). Do not output random gibberish words.\n2. For any numeric identifier fields (such as "id", "genre_id", or other ID / index fields, especially inside arrays): DO NOT generate huge random numbers or floats (e.g., 50414785.24). Instead, they MUST start from 1 and increment sequentially (1, 2, 3, etc.) for each item in the array.\n3. Ensure the generated values across all objects/arrays are coherent, logically connected, and consistent with each other. The order items, prices, user details, and addresses should be logically related and make sense together as a single realistic scenario. Do not generate random mismatched objects.\n4. Use typical Indonesian names (e.g., Budi, Agus, Siti, Dewi, Joko) and real Indonesian locations (e.g., Jakarta, Bandung, Surabaya, Jakarta Selatan, etc.) for name and address fields.\n5. The data must highly align with the provided API Path and Description context.\n6. ${arrayInstructions}\n\nReturn ONLY the JSON object. Do not wrap in markdown code blocks.`
