...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `Generate realistic mock JSON data that strictly matches this JSON Schema:\n${JSON.stringify(schema, null, 2)}\n\nContext:\n- HTTP Method: ${method || 'GET'}\n- API Path: ${path || '/'}\n- Description: ${description || ''}\n\nMake sure the generated data is highly realistic, contextual, and matches the names and purposes of the fields. For example, use realistic names for fields like name, email, street, description, title, price, status, etc. based on the API endpoint description and path.\n\nReturn ONLY the JSON object. Do not wrap in markdown code blocks.`
